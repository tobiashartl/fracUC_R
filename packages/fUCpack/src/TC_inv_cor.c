...
SEXP TC_inverse_corr(SEXP S0t, SEXP S0c, SEXP sigma_eta, SEXP sigma_eps,
                     SEXP cov_etaeps, SEXP y, SEXP n, SEXP ma, SEXP ma_c){
    ...
    double * maP = REAL(ma);
    double a = REAL(sigma_eta)[0];
    double b = REAL(sigma_eps)[0];
    double c = REAL(cov_etaeps)[0];
    ...
    /* in Xacc, we iteratively store BB' + nu SS' */
    Ytt = (double *) R_alloc(N * 1, sizeof(double));

    /* initialize it to zero */
    for (i = 0; i< N; i++) Ytt[i] = 0.0;
    ...
    for (t = 1; t < N+1; t++){
        ...
        for ( i = 0; i < k; i++){
            ...
            for (j = 0; j < i+1; j++){
                tempYx += S0cP[j * N + i] * yP[j];
            }
            Ytt[i] = tempYx;
        }


        /* Generate upper triangular a %*% S0cP + c %*% S0tP */
        for(i = 0; i < k; i++){
            for(j = 0; j < i+1; j++){
                Bacc[i * N + j] = a * S0cP[j * N + i] + c * S0tP[j * N + i];
                /*if(t==6) Rprintf("Bacc[%i,%i]: %f\n", i,j, Bacc[i * N + j]);*/
            }
        }
