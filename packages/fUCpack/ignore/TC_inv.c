...
SEXP TC_inverse(SEXP S0t, SEXP S0c, SEXP nu, SEXP y, SEXP n, SEXP ma, SEXP ma_c){
    ...
    double * Xacc, *X, *Sacc, *Bacc, *Yc, *Yt, *X1y, *X2y;
    ...
    /* in Xacc, we iteratively store BB' + nu SS' */
    Sacc = (double *) R_alloc(N * N, sizeof(double));

    /* initialize it to zero */
    for (i = 0; i< N*N; i++) Sacc[i] = 0.0;
    ...
    /* in Xacc, we iteratively store BB' + nu SS' */
    X1y = (double *) R_alloc(N * 2, sizeof(double));

    /* initialize it to zero */
    for (i = 0; i< N*2; i++) X1y[i] = 0.0;

    /* in Xacc, we iteratively store BB' + nu SS' */
    X2y = (double *) R_alloc(N * 1, sizeof(double));

    /* initialize it to zero */
    for (i = 0; i< N; i++) X2y[i] = 0.0;
    ...
    for (t = 1; t < N+1; t++){
        ...
        for (i = 0; i < k; i++){
            for (j = i; j < k; j++){
                ...
                if (i == 0){
                    ...
                    Sacc[ j * N + i] = Sacc[ (j-1) * N + i - 1] + S0tP[i] * S0tP[j];
        ...
        for ( i = 0; i < k; i++){
            ...
            for (j = 0; j < k; j++){
                if(i > j){
                    tempYc += a * Sacc[i * N + j] * yP[k - j - 1];
                    tempYx += Bacc[i * N + j] * yP[k - j - 1];
                }else{
                    tempYc += a * Sacc[j * N + i] * yP[k - j - 1];
