...
# Wd, etc
setwd("/Users/tobias/Dokumente/Projekte/filtering unknown persistence/R/data")
...
for(p in c(0:12)){
    ...
    if(corr){
        ...
        #Sigma <- cbind(runif(R, 0.0001, 0.001),
        #               runif(R, 0.001, 0.005))
        #Sigma <- cbind(Sigma, sqrt(Sigma[,1]) * sqrt(Sigma[,2]) * runif(R, -0.5, 0.5))
        #Sigvec <- apply(Sigma, 1, function(x) mlogvech(matrix(c(x[1], x[3], x[3], x[2]), 2, 2)))
    ...
    optfn <- function(par0, y){ tryCatch({
        est <- optim(par=par0, method = "BFGS", 
                     fn = UC_i2_opt_ML, deterministics = det,  
    ...
    RESULTS <- t(RESULTS)
    save(RESULTS, file = paste("./app/Benchmark_ML_i2_cor_p", p, "_trend.RData", sep=""))
...
for(p in c(2:12)){
    cat("p = ", p, "\n")
    load(file = paste("./app/Benchmark_ML_i2_cor_p", p, "_trend.RData", sep=""))
...
det <- UC_i2_opt_ML(theta, y, nulim = c(0, Inf), 
                 corr=TRUE, deterministics = T, START=2, diffuse = T, 
                 return.det = TRUE, nu.opt=F, 
                 eta=NULL, Q.trans = "mlv", flip=F, neg=F, pq=c(p, 0), penalty.corr=F, 
                 seas=F, period=12, irregular=F)
n <- length(y)
Z <- matrix(frac_diff(rep(1, n), -2), n)
...
TC <- fUC_KS_ARMA_approx(c(2, theta), y - Z %*% det, nulim = c(0, Inf), 
...
# replace estimated likelihood by that of the fractional model for a better comparison
...
SSR <- (fUC_comp((y - dettrend), 2, Q, theta[-(1:3)], corr=TRUE)$v[-1])^2 %>% sum(.)
...
colnames(EST)<- c("par", "se")
saveRDS(EST, file = "./app/Benchmark_i2_ML_Results.RDS")
...
saveRDS(data.plot, file = "./app/Benchmark_i2_ML_TC.RDS")
