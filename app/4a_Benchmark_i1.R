...
for(p in c(2:12)){
    ...
    optfn <- function(par0, y){ tryCatch({
        est <- optim(par=par0, method = "BFGS", 
                     fn = UC_opt_ML, deterministics = TRUE,  
    ...
    RESULTS <- t(RESULTS)
    save(RESULTS, file = paste("./app/Benchmark_ML_i1_cor_p", p, "_trend.RData", sep=""))
    RESULTS.win <- RESULTS[which.min(RESULTS[,1]), ]
    
    
    cat("Results: det = ", det, ", p = ", p, "\n", "LL = ", RESULTS.win[1], ", theta = ", 
        RESULTS.win[2], ", corr = ", cov2cor(mlogvech2mat(RESULTS.win[3:5]))[2,1], "\n")
}
...
for(p in c(2:12)){
    cat("p = ", p, "\n")
    load(file = paste("./app/Benchmark_ML_i1_cor_p", p, "_trend.RData", sep=""))
...
colnames(EST)<- c("par", "se")
saveRDS(EST, file = "./app/Benchmark_i1_ML_Results.RDS")
...
saveRDS(data.plot, file = "./app/Benchmark_i1_ML_TC.RDS")
